local maker = loadstring(game:HttpGet("https://raw.githubusercontent.com/t4tuya/uiLibutil/main/instancemaker.lua"))();
...
function Library.Main(text : string)
    ...
    spawn(function()
        wait()
        SaveProperties(game.CoreGui.Tatuyalib,allproperties)
        Main.Visible = false
        for c,x in next,allproperties do
            for b,n in next,allproperties[c] do
                c[b] = 1
        ...
        spawn(function()
            while task.wait(0.05) do
                if Main.BackgroundTransparency == 0 then
                    allproperties = {}
                    SaveProperties(game.CoreGui:FindFirstChild("Tatuyalib"),allproperties)
                    if not game.CoreGui:FindFirstChild("Tatuyalib") then
                        break;
                    end
                end
            end
